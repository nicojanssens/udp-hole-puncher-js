...
class UdpHolePuncher extends events.EventEmitter {
  ...
  connect(addr, port) {
    ...
    this.sendRequestInterval = setInterval(() => {
      if (self.attempts > 0) {
        ...
        self.log.error(`failed to connect with ${addr}:${port}`);
        clearInterval(self.sendRequestInterval);
        self.restoreSocket();
        self.emit('timeout');
      }
    }, this.timeout);
  ...
  onMessage() {
    const self = this;
    return (bytes, rinfo) => {
      self.log.debug(`receiving message from ${JSON.stringify(rinfo)}`);
  ...
  initSocket() {
    ...
    this.messageListeners.forEach((callback) => {
      self.socket.removeListener('message', callback);
    });
    this.errorListeners.forEach((callback) => {
      self.socket.removeListener('error', callback);
    });
